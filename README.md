 ...
 **=> By making the method async, will not block the current thread until the db operation finishes. Using an async method with await, returns the thread to the pool while de I/O operation (db) is happening. When the db operation is finished, it continues. This allows the server to handle more requests at the same time by not blocking threads on I/O operations.**
 <br>
...
**=> In this example I've implemented a PaymentProviderResolver to dinamically resolve the payment provider implementations in runtime, based on the name input on the POST Rentals request. This will provide extensability and is designed according to SOLID principles. The implementation is based on interfaces, and it is possible to add a new payment provider by simply creating a new payment provider class and implementing the IPaymentProvider interface.**
...
throwing an exception that will be treated on the exception handling middleware. A different approach could be used: for instance, using a result object as return, with a succeeded true/false). This way there's no need to use exceptions to control flow.**
